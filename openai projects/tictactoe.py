...
def draw_status():
    ...
    screen.fill((0, 0, 0), (0, 400, WIDTH, 100))  # Fill the status area
    text_rect = text.get_rect(center=(WIDTH / 2, HEIGHT + 50))  # Center text in status area
...
def drawXO(row, col):
    ...
    pos_x = (row - 1) * HEIGHT / 3 + HEIGHT / 6  # Calculate center position
    pos_y = (col - 1) * WIDTH / 3 + WIDTH / 6

    grid[row - 1][col - 1] = current_player
    if current_player == 'x':
        pg.draw.line(screen, (0, 0, 0), (pos_y - size // 2, pos_x - size // 2), (pos_y + size // 2, pos_x + size // 2), 5)
        pg.draw.line(screen, (0, 0, 0), (pos_y - size // 2, pos_x + size // 2), (pos_y + size // 2, pos_x - size // 2), 5)
        current_player = 'o'
    else:
        center = (pos_y, pos_x)
...
def user_click():
    ...
    col = x // (WIDTH // 3) + 1  # Corrected column calculation
    row = y // (HEIGHT // 3) + 1  # Corrected row calculation

    if (row and col and grid[row - 1][col - 1] is None) and current_winner is None and not is_draw:
        drawXO(row, col)
        check_win()
...
def reset_game():
    ...
    current_winner = None
    grid = [[None] * 3, [None] * 3, [None] * 3]
    game_initiating_window()  # Redraw the grid
...
while True:  # continue loop until user closes the window
    for event in pg.event.get():  # check for new events
        if event.type == QUIT:
            ...
            user_click()
            if current_winner is not None or is_draw:  # Check for game over
                reset_game()
    pg.display.update()
    clock.tick(FPS)